...
- **Staging deploys via cPanel Git, not GitHub Actions.** o2switch blocks inbound port 22 (SSH/SFTP) by IP allowlist, and GitHub Actions runner IPs rotate too fast to whitelist, so `deploy-staging.yml`'s `lftp` push can never succeed there — it's superseded for staging (still present but expected to fail; ask before touching further). Staging now works by pulling instead of pushing: o2switch's cPanel "Git Version Control" clones this repo (public, HTTPS, no auth needed) and tracks the `staging` branch; after pushing to `staging`, go into cPanel and run "Update from Remote" then "Deploy HEAD Commit" (or the equivalent "Pull or Deploy" action) to publish. `.cpanel.yml` at the repo root wires the deploy path and calls `scripts/cpanel-deploy-staging.sh`, which reproduces the old workflow's payload prep (noindex injection, GA-block stripping, sitemap removal, `.htaccess-staging` → `.htaccess` swap) in a scratch copy before rsyncing into the live docroot — it never mutates the tracked git checkout. `.htpasswd` is managed out of band (`scripts/htpasswd-gen.sh`, uploaded manually) since the deploy no longer runs in CI where the `STAGING_HTPASSWD_*` secrets live.
- Production still deploys via `.github/workflows/deploy-production.yml` (`lftp mirror` over SFTP) — untested since the o2switch IP-allowlist issue was found, and will hit the exact same port-22 block if pushed to `main` today. Needs its own cPanel Git repo + `.cpanel.yml` (different deploy path, no noindex/GA stripping) before relying on it; not yet set up.
