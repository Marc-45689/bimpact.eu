...
- **Both staging and production deploy via cPanel Git, not GitHub Actions.** o2switch blocks inbound port 22 (SSH/SFTP) by IP allowlist, and GitHub Actions runner IPs rotate too fast to whitelist, so `deploy-staging.yml` and `deploy-production.yml`'s `lftp` pushes can never succeed there — both are superseded (still present as `workflow_dispatch` only; ask before touching further). Instead, o2switch's cPanel "Git Version Control" pulls: two separate cPanel repos clone this repo (public, HTTPS, no auth needed), one tracking `staging` (deploys to `staging.bimpact.eu`) and one tracking `main` (deploys to `bimpact.eu`, docroot `/home/foma0863/public_html/`). After pushing to either branch, go into that repo's entry in cPanel and run "Update from Remote" then "Deploy HEAD Commit" (or the equivalent "Pull or Deploy" action) to publish.
- `.cpanel.yml` at the repo root is identical on both branches (`- /bin/bash scripts/cpanel-deploy.sh`) so merging `staging` into `main` never conflicts on that file. `scripts/cpanel-deploy.sh` reads which branch is actually checked out on the host and dispatches to `scripts/cpanel-deploy-staging.sh` (noindex injection, GA-block stripping, sitemap removal, `.htaccess-staging` → `.htaccess` swap) or `scripts/cpanel-deploy-production.sh` (no transforms — ships the real `.htaccess`/sitemap as committed), both of which do the work in a scratch copy before rsyncing into the live docroot — it never mutates the tracked git checkout. `.htpasswd` (staging-only; production has no Basic Auth) is managed out of band (`scripts/htpasswd-gen.sh`, uploaded manually) since the deploy no longer runs in CI where the `STAGING_HTPASSWD_*` secrets live.
...
- Site content is trilingual: French at the root (canonical/default, `x-default`), English mirrored under `/en/`, Spanish mirrored under `/es/` (added 2026-09-16). FR directory names come from `.claude/i18n-dirs.json` (`a-propos`, `projets`, `blog`, `mentions-legales/confidentialite|cgu|cookies` — matches the starter's default `about`→`a-propos` mapping; the FR display label is "À propos", briefly renamed to "Qui suis-je"/`qui-suis-je` and reverted back on 2026-09-16); the EN tree mirrors them under `/en/about/`, `/en/projects/`, `/en/blog/`, `/en/legal/privacy|terms|cookies/`; the ES tree mirrors them under `/es/acerca/`, `/es/blog/`, `/es/legal/privacidad|terminos|cookies/` (same `i18n-dirs.json` `"es"` mapping — no `/es/proyectos/` yet, there's no `/projets/` page live on any locale right now). ES copy is written formal ("usted"), matching FR's `vous`.
- Every page carries reciprocal `<link rel="alternate" hreflang="...">` tags (`fr`, `en`, `es`, `x-default`=fr) pointing at its exact counterpart — never just the homepages. The nav's `.nav-lang` switcher (see `docs/components.md`) links to that same counterpart, now a three-way FR/EN/ES toggle.
- New pages: create the FR version, then its `/en/` and `/es/` twins, wire the switcher hrefs both ways for all three, and add the page to `scripts/sitemap-update.py`'s `LOCALES` maps if it introduces a whole new top-level category (not needed for an ordinary page/article/project under an existing category).
